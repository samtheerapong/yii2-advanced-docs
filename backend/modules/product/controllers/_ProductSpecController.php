...
class ProductSpecController extends Controller
{
    ...
    {
        $model = $this->findModel($id);
        $folder = $model->uploadFolders['spec'];
        $filePath = Yii::getAlias('@webroot') . '/' . $folder . '/' . $fileName;

        if (unlink($filePath)) {
            // Remove the file name from the database attribute
            // Update the model attribute and save the model
            // Redirect back to the update or view page
        } else {
